...
iteration = 1;
k =10;
C_value = [0.5, 5.0, 30.0];
epsilon = [0.5, 3.0, 10.0];
poly_order = [2,3];

for i = 1:length(C_value)
    for j =1:length(epsilon)
        for l =1:length(poly_order)
            ...
            for m =1:k
                ...
                SVM = fitrsvm(training_input, training_target,'KernelFunction', 'polynomial', 'PolynomialOrder', poly_order(l),...
                'BoxConstraint', C_value(i), 'Epsilon', epsilon(j),'Standardize', true );
            ...
            hyper_data.PolyOrder = poly_order(l);
            hyper_data.epsilon = epsilon(j);
...
for i =1:k
    ...
    SVM = fitrsvm(training_input, training_target,'KernelFunction', 'polynomial', 'PolynomialOrder', bestHP.poly_order,...
